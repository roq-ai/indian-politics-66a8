...
import { ProfileUsageListPage as ShowCaseList } from 'pages/profile-usages';

function DashboardPage() {
  const { session } = useSession();

  const politicianStat = useSWR<PaginatedInterface<PoliticianInterface>>(
    () => `/politicians`,
    () => getPoliticians({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Politicians',
      result: politicianStat?.data?.totalCount,
      isLoading: politicianStat.isLoading,
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Profile Usages
                </Text>
                <NextLink href={`profile-usages`} passHref legacyBehavior>
                  <Button
