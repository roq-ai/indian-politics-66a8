...
import { getProfileUsageById, updateProfileUsageById } from 'apiSdk/profile-usages';
...
function ProfileUsageEditPage() {
  ...
  const handleSubmit = async (values: ProfileUsageInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateProfileUsageById(id, values);
      mutate(updated);
      resetForm();
